...
/**
 * {@link ExternalTerminologyValidation} that supports PHIR (Public Health Information
 * Repository) terminology validation.
 *
 * <p>Mirrors the architecture of {@link FhirTerminologyValidation}. The PHIR wire format
 * is not yet finalized, so request/response handling is structured to parallel FHIR but
 * pieces that depend on the unfinalized spec are marked with {@code TODO(PHIR spec):}
 * comments rather than committing to guessed URL paths or payloads.
 */
public class PhirTerminologyValidation implements ExternalTerminologyValidation {
    private static final Logger LOG = LoggerFactory.getLogger(PhirTerminologyValidation.class);
    private final String baseUrl;
    private final boolean failOnError;
    private final WebClient webClient;

    public PhirTerminologyValidation(String baseUrl) {
        this(baseUrl, true);
    }

    public PhirTerminologyValidation(String baseUrl, boolean failOnError) {
        this(baseUrl, failOnError, WebClient.create());
    }

    public PhirTerminologyValidation(String baseUrl, boolean failOnError, WebClient webClient) {
        this.baseUrl = baseUrl;
        this.failOnError = failOnError;
        this.webClient = webClient;
    }

    // TODO(PHIR spec): confirm query-parameter name for value-set / code-system URLs once
    // the PHIR request schema is finalized. Assuming "url=" to parallel FHIR for now.
    private String extractUrl(String referenceSetUri) {
        UriComponents uriComponents =
                UriComponentsBuilder.fromUriString("/phir?" + referenceSetUri).build();
    ...
    private WebClient buildRestClientCall(String url) {
        ...
        // TODO(PHIR spec): replace "application/phir+json" with the canonical PHIR media
        // type once the spec publishes one.
        return webClient
                .mutate()
                .clientConnector(new ReactorClientHttpConnector(client))
                .baseUrl(url)
                .defaultHeader(HttpHeaders.ACCEPT, "application/phir+json")
                .build();
    }
    ...
    protected DocumentContext internalGet(String uri) throws WebClientException {
        ...
        ResponseEntity<String> respEntity = Optional.ofNullable(mono.block())
                .orElseThrow(
                        () -> new InternalServerException("could not connect to external PHIR Terminology Server"));
        ...
        if (statusCode.is2xxSuccessful()) {
            ...
            throw new ExternalTerminologyValidationException(
                    "Error response received from the PHIR terminology server. HTTP status: %s. Body: %s"
                            .formatted(statusCode, responseBody));
        }
    }

    // TODO(PHIR spec): finalize endpoint paths. Mirroring FHIR shape for now; the actual
    // PHIR endpoints (e.g. CodeSystem / ValueSet lookups) may use different path segments
    // or verbs.
    ...
    private static final String ERR_SUPPORTS =
            "An error occurred while checking if PHIR terminology server supports the referenceSetUri: %s";
    static final String CODE_PHRASE_TEMPL = "code=%s&system=%s";
    private static final String ERR_EXPAND_VALUESET = "Error while expanding ValueSet[%s] via PHIR";
    ...
    // TODO(PHIR spec): replace the placeholder accepted-API prefixes below with the
    // canonical PHIR service-api identifiers once published. The entries here are
    // placeholders that let us wire things end-to-end without committing to a final URL.
    @SuppressWarnings({"serial"})
    private final Set<String> acceptedPhirApis = new HashSet<>() {
        {
            add("//phir.local");
            add("terminology://phir.local");
        }
    ...
    private boolean isValidTerminology(Optional<String> url) {
        ...
        return acceptedPhirApis.stream()
    ...
    public boolean supports(TerminologyParam param) {
        ...
        try {
            // TODO(PHIR spec): the "$.total" JSON path mirrors FHIR Bundle semantics;
            // replace with the PHIR lookup-count selector once the response schema lands.
    ...
    public List<DvCodedText> expand(TerminologyParam param) {
        ...
        if (param.getServiceApi().isEmpty() || !isValidTerminology(param.getServiceApi())) {
            LOG.warn("Unsupported PHIR service-api: {}", param.getServiceApi());
    ...
    // TODO(PHIR spec): adjust JSON path selectors once the PHIR expansion payload
    // shape is finalized. Mirrors the FHIR ValueSet $expand response for now.
    ...
    private Try<Boolean, ConstraintViolationException> validateCode(String url, CodePhrase codePhrase) {
        ...
        try {
            // TODO(PHIR spec): confirm the validate-code endpoint + query-parameter
            // shape. Mirroring FHIR's "/CodeSystem/$validate-code?url=...&code=..." path.
            context = internalGet(
                    baseUrl + "/CodeSystem/$validate-code?url=" + url + "&code=" + codePhrase.getCodeString());
        } catch (WebClientException e) {
            if (failOnError)
                throw new ExternalTerminologyValidationException(
                        "An error occurred while validating the code in CodeSystem via PHIR", e);
            LOG.warn("An error occurred while validating the code in CodeSystem via PHIR: {}", e.getMessage());
            return Try.success(Boolean.FALSE);
        }
        // TODO(PHIR spec): confirm response shape. Mirroring FHIR Parameters response.
    ...
    private Try<Boolean, ConstraintViolationException> expandValueSet(String url, CodePhrase codePhrase) {
        DocumentContext context;
        try {
            // TODO(PHIR spec): confirm expand endpoint path once finalized.
            context = internalGet(baseUrl + "/ValueSet/$expand?url=" + url);
        } catch (WebClientException e) {
            if (failOnError)
                throw new ExternalTerminologyValidationException(
                        "An error occurred while expanding the ValueSet via PHIR", e);
            LOG.warn("An error occurred while expanding the ValueSet via PHIR: {}", e.getMessage());
            return Try.success(Boolean.FALSE);
        }
